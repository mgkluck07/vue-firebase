  ...
  methods: {
    ...
    async cargarDatos() {
      ...
      const teatro = {
        ...
        nombre: 'Teatro de la Super Opera',
      ...
      const obraHamilton = {
        oid: 'hamilton',
        titulo: 'Hamilton',
        portada: 'https://firebasestorage.googleapis.com/v0/b/teatro-super-opera-fdafd.appspot.com/o/obras%2Fhamilton%2Fhamilton.png?alt=media&token=b2550a4a-10a1-4f33-8671-3486eca9ebab',
        descripcion: '',
        categorias: [
      ...
      const obraReyLeon = {
        oid: 'rey-leon',
        titulo: 'Rey Leon',
        portada: 'https://firebasestorage.googleapis.com/v0/b/teatro-super-opera-fdafd.appspot.com/o/obras%2Fel-rey-leon%2Fel-rey-leon.png?alt=media&token=2d2f6cfd-ff3e-4615-8476-b96d72631cda',
        descripcion: '',
        categorias: [
      ...
      const obraFrozen = {
        oid: 'frozen',
        titulo: 'Frozen',
        portada: 'https://firebasestorage.googleapis.com/v0/b/teatro-super-opera-fdafd.appspot.com/o/obras%2Ffrozen%2Ffrozen.png?alt=media&token=2eaa53cb-55f2-499d-bda2-4ccd31a0e9ab',
        descripcion: '',
        categorias: [
      ...
      const obraAnastansia = {
        oid: 'anastasia',
        titulo: 'Anastasia',
        portada: 'https://firebasestorage.googleapis.com/v0/b/teatro-super-opera-fdafd.appspot.com/o/obras%2Fanastasia%2Fanastasia.png?alt=media&token=3c2a3462-e42d-4091-80f7-7cd217ce932b',
        descripcion: '',
        categorias: [
          { categoria: 'A', precio: 350 },
      ...
      for(let i = 1; i<=10;i++){
        ...
        obraReyLeon.fechas.push(fecha02);
        obraReyLeon.fechas.push(fecha03);
        obraFrozen.fechas.push(fecha04);
        obraAnastansia.fechas.push(fecha05);
        ...
        let presentacion02 = {
          pid: `${obraReyLeon.oid}-${teatro.tid}-${generarFormatoFecha(fecha02, '-', true)}`,
          fecha: fecha02,
          teatro: {tid: teatro.tid, nombre: teatro.nombre},
          obra: {oid: obraReyLeon.oid, titulo: obraReyLeon.titulo}
        };

        let presentacion03 = {
          pid: `${obraReyLeon.oid}-${teatro.tid}-${generarFormatoFecha(fecha03, '-', true)}`,
          fecha: fecha03,
          teatro: {tid: teatro.tid, nombre: teatro.nombre},
          obra: {oid: obraReyLeon.oid, titulo: obraReyLeon.titulo}
        };
        ...
        let presentacion05 = {
          pid: `${obraAnastansia.oid}-${teatro.tid}-${generarFormatoFecha(fecha05, '-', true)}`,
          fecha: fecha05,
          teatro: {tid: teatro.tid, nombre: teatro.nombre},
          obra: {oid: obraAnastansia.oid, titulo: obraAnastansia.titulo}
        };

        batch.set(db.collection('obras').doc(obraHamilton.oid).collection('presentaciones').doc(presentacion01.pid), presentacion01);
        batch.set(db.collection('obras').doc(obraReyLeon.oid).collection('presentaciones').doc(presentacion02.pid), presentacion02);
        batch.set(db.collection('obras').doc(obraReyLeon.oid).collection('presentaciones').doc(presentacion03.pid), presentacion03);
        batch.set(db.collection('obras').doc(obraFrozen.oid).collection('presentaciones').doc(presentacion04.pid), presentacion04);
        batch.set(db.collection('obras').doc(obraAnastansia.oid).collection('presentaciones').doc(presentacion05.pid), presentacion05);
      ...
      batch.set(db.collection('obras').doc(obraAnastansia.oid), obraAnastansia);
      this.mostrarOcupado({titulo:'Cargando Datos',mensaje: 'Cargando datos...'});
      try {
        batch.commit();
        this.mostrarExito('Se cargo la información correctamente.');
      } catch (error) {
        this.mostrarError('Hubo un error al querer cargar la información.');
      } finally {
        this.ocultarOcupado();
      }
    }
