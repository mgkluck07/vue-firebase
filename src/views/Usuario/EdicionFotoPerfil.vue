                      ...
                      <v-flex xs6>
                        <v-layout justify-end>
                          <v-icon
                            v-if="foto.url != usuario.fotoPerfil"
                            medium
                            @click="preguntaEliminarImagen(foto)"
                            >delete</v-icon
                          >
                        </v-layout>
                      </v-flex>
            ...
            <v-dialog v-model="preguntaEliminar" max-width="400" persistent>
              <v-card>
                <v-card-text class="title">
                  La foto se eliminara permanentemente, ¿Deseas Continuar?
                </v-card-text>
                <v-btn color="secondary" flat @click="preguntaEliminar = false"
                  >Cancelar</v-btn
                >
...
export default {
  ...
  methods: {
    ...
    async consultarFotosPerfil() {
      this.mostrarOcupado({
        titulo: "Consultado galería",
        mensaje: "Recuperando galería de imágenes",
      });
      try {
        ...
        this.mostrarError("Ocurrió un error recuperando la galería de fotos.");
      } finally {
        this.ocultarOcupado();
      }
    },
    ...
    async guardarImagen() {
      ...
      try {
        ...
        this.$router.push({
          name: "perfil",
          params: { userName: this.usuario.userName },
        });
      } catch (error) {
        this.mostrarError("Ocurrio un error almacenando la imagen");
      } finally {
        this.ocultarOcupado();
      }
    },
    async seleccionarFotoPerfil(url) {
      this.mostrarOcupado({
        titulo: "Actualizando imagen",
        mensaje: "Estableciendo foto de perfil",
      });
    ...
    async eliminarImagen() {
      ...
      this.mostrarOcupado({
        titulo: "Eliminando imagen",
        mensaje: "Se esta eliminando tu imagen ...",
      });
      try {
        await db
          .collection("usuarios")
        ...
        this.mostrarError("Ocurrió un error eliminando tu imagen.");
      } finally {
        this.ocultarOcupado();
      }
    },
